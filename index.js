...
function arrObjKeys(obj, inspect, indentation, inNewLine) {
  ...
  if (isArr) {
    ...
    for (var i = 0; i < obj.length; i++) {
      xs[i] = has(obj, i) ? inspect(obj[i], obj, indentation, inNewLine) : '';
    }
  }
  ...
  for (var key in obj) { // eslint-disable-line no-restricted-syntax
    ...
    if ((/[^\w$]/).test(key)) {
      xs.push(inspect(key, obj) + ': ' + inspect(obj[key], obj, indentation, inNewLine));
    } else {
      xs.push('"' + key + '": ' + inspect(obj[key], obj, indentation, inNewLine));
    }
  }
...
module.exports = function inspect_(obj, options, depth, seen, indentation, inNewLine) {
  ...
  function inspect(value, from, indentation, inNewLine) {
    if (from) {
      seen = seen.slice();
      seen.push(from);
    }

    return inspect_(value, opts, depth + 1, seen, indentation, inNewLine);
  }
  ...
  if (isError(obj)) {
    var parts = arrObjKeys(obj, inspect, 0, false);
    if (parts.length === 0) {
      return '[ ' + String(obj) + ' ]';
    }
  ...
  if (isMap(obj)) {
    ...
    mapForEach.call(obj, function (value, key) {
      mapParts.push(
        inspect(key, obj, indentation, newLine) + ' => ' + inspect(value, obj, indentation, newLine)
      );
    });
  ...
  if (isSet(obj)) {
    ...
    setForEach.call(obj, function (value) {
      setParts.push(inspect(value, obj, indentation, newLine));
    });
  ...
  if (isNumber(obj)) {
    return markBoxed(inspect(Number(obj), undefined, 0, false));
  }

  if (isBigInt(obj)) {
    return markBoxed(inspect(bigIntValueOf.call(obj), undefined, 0, false));
  }
  ...
  if (isString(obj)) {
    return markBoxed(inspect(String(obj), undefined, 0, false));
  }
